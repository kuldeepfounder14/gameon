...
const PointsRummyHome = () => {
  ...
  // socket
  useEffect(() => {
    const stored = sessionStorage.getItem("roomData");
    if (stored) {
  ...
  useEffect(() => {
    if (roomData) {
      ...
      if (task === "room_destroyed") {
        // console.log("room destroyed")
        navigate("/ptmenu");
        return;
      } else if (task === "first_response") {
      ...
      } else if (task === "toss_result") {
        setMessage("");
        roomData?.response?.players?.forEach((player, i) => {
          sessionStorage.setItem(
            `ptRummyPlayerTossCard${i + 1}`,
            player?.toss_card?.id || {}
          );
        });
        ...
        if (countdown === 1) {
          const tossWinner = sessionStorage.getItem("ptRummyTossWinner");
      ...
      } else if (task === "joker_and_discarded") {
        ...
        setCurrentPoints(79);
      } else if (task === "card_drawn_from_deck") {
        ...
        setShowRemainingCards(roomData?.response?.leftDeck || null);
      }  else if (task === "card_drawn_from_discard") {
        const sortedResults=roomData?.response?.results[userId] || null;
        setSortedResult(sortedResults)
       
      } else if (roomData?.response?.task === "turn_started") {
        const sortedResults=roomData?.response?.results[userId] || null;
        setSortedResult(sortedResults)
          ...
          if (turnPlayer.user_id === Number(userId)) {
            ...
            eliminateTimerRef.current = setTimeout(() => {
              ...
              if (roomId && userId && !hasPickedCard) {
                ...
                PointsRummySocket.emit("eliminate", {
                  roomId,
        ...
        //   navigate("/");
        // }, 2000);
      } else if (task === "wait_for_opponent_declare") {
        const sortedResults=roomData?.response?.results[userId] || null;
        setSortedResult(sortedResults)
        const turnPlayer = roomData?.response?.players.find(
          (p) => p.is_turn === 1
        ...
        setLockMessageShown(false);
        roomData?.response?.players?.forEach((player, i) => {
          sessionStorage.removeItem(`ptRummyPlayerTossCard${i + 1}`);
        });
      }
  ...
  const discardCardFromHand = (cardToDiscard) => {
    ...
    PointsRummySocket.emit("discardCard", payload);
    // setHasPickedCard(true);
    setCardDiscardedFromHand(cardToDiscard);
    // console.log(
    //   "discardCarddiscardCarddiscardCarddiscardCard payload",
    //   payload
    // );
  };
  ...
  const pickFromDiscardPile = () => {
    ...
    PointsRummySocket.emit("selectCardsFromDiscardedCards", payload);
    setDiscardPile(null);
  ...
  const pickFromLeftDeck = () => {
    ...
    PointsRummySocket.emit("selectCardsFromLeftCards", payload);
    setHasPickedCard(true); // After successful pick
  };
  // declare
  const handleDeclare = () => {
    ...
    PointsRummySocket.emit("declare", payload);
    setDeclareAfterFinally(false);
  ...
  console.log(
    "currentTurnUserIdcurrentTurnUserIdcurrentTurnUserId",
    sortEnabled,sortedResult
  );
...
export default PointsRummyHome;
