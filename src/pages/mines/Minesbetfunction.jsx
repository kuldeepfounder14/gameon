...
function Minesbetfunction({
  ...
  const selectBetAmount = (amount) => {
    ...
    setBetAmount(Number(amount));
    setShowPopup(false);
  };
console.log(" betAmount + MultiplierIndex.size * multi", betAmount ,MultiplierIndex.size , multi)
    console.log(" betAmount + MultiplierIndex.size * multi",typeof betAmount ,typeof MultiplierIndex.size ,typeof multi)
  // Calculate current cashout value
  const currentCashOut = Number(
    Number(betAmount) + MultiplierIndex.size * Number(multi)
  ).toFixed(2);
