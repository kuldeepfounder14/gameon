...
function DLGameMenu() {
  ...
  useEffect(() => {
    DealsRummySocket.on("connect", () => {
      console.log("✅ Socket connected:", DealsRummySocket.id);
    });

    DealsRummySocket.on("connect_error", (err) => {
      console.error("❌ Socket connection error:", err.message);
    });

    return () => {
      DealsRummySocket.off("connect");
      DealsRummySocket.off("connect_error");
    };
  }, []);
  ...
  const GamePlayHandler = () => {
   // console.log("fsdfdsfd")
    ...
    const payload = { userId, gameId: String(toggleGameType),noOfDeals: toggleDealsType};
    DealsRummySocket.emit("deal_room", payload);
   // console.log("fsdfdsfd2222")
    DealsRummySocket.on("deals_game_response", (data) => {
    //  console.log("connected")
     // console.log("datadata",data)
      sessionStorage.setItem("roomData_deals", JSON.stringify(data));
      navigate(`/dealsrummy/${toggleGameType}`);
    });
    DealsRummySocket.on("deal_insufficient_balance", (data) => {
      // console.log("connected")
  ...
  return (
    ...
    >
      <header className="px-4 py-2 flex justify-between items-center">
        ...
        <p className="font-serif">Deals Rummy </p>
