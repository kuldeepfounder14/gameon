...
const DealsRummyHome = () => {
  ...
  // const [nextRoundModalVisible, setNextRoundModalVisible] = useState(false);
  // tie case modal isseu in points and deals game
  // getting declare button isseu two after first deals
  // sorting
  // check for correc or not totalpoints and each round points
  // const handleSort = () => {
  //   if (!userResult?.cards) return;
  //   const result = analyzeHand(userResult.cards);
  ...
  // socket
  useEffect(() => {
    const stored = sessionStorage.getItem("roomData_deals");
    if (stored) {
  ...
  useEffect(() => {
    if (roomData) {
      ...
      if (task === "deal_room_destroyed") {
        // console.log("room destroyed")
        navigate("/dlmenu");
        return;
      } else if (task === "deal_first_response") {
        setMessage("Waiting for other players...");
        setCurrentPoints(0);
      } else if (
        task === "deal_loading_timer" &&
        countdown !== null &&
        timerStatus === 1
      ) {
        setCurrentPoints(0);
      ...
      } else if (task === "deal_toss_result") {
        setMessage("");
        roomData?.response?.players?.forEach((player, i) => {
          sessionStorage.setItem(
            `dlRummyPlayerTossCard${i + 1}`,
            player?.toss_card?.id || {}
          );
        });
        ...
        roomData?.response?.players?.forEach((player, i) => {
          ...
          if (player?.toss_result === 1) {
            //  sessionStorage.setItem(`dlRummyTossWinner`,player?.user_id || {});
            sessionStorage.setItem(
              `dlRummyTossWinner`,
              player?.user_name || {}
            );
        ...
        if (countdown === 1) {
          const tossWinner = sessionStorage.getItem("dlRummyTossWinner");
      ...
      } else if (task === "deal_joker_and_discarded") {
        ...
        setCurrentPoints(79);
      } else if (task === "deal_card_drawn_from_deck") {
      ...
      }else if (task === "deal_card_drawn_from_discard") {
        const sortedResults=roomData?.response?.results[userId] || null;
        setSortedResult(sortedResults)
       
      }  else if (roomData?.response?.task === "deal_turn_started") {
          const sortedResults=roomData?.response?.results[userId] || null;
        setSortedResult(sortedResults)
          ...
          if (turnPlayer.user_id === Number(userId)) {
            ...
            eliminateTimerRef.current = setTimeout(() => {
              ...
              if (roomId && userId && !hasPickedCard) {
                ...
                DealsRummySocket.emit("deal_eliminate", {
                  roomId,
        ...
        //   navigate("/");
        // }, 2000);
      } else if (task === "deal_wait_for_opponent_declare") {
          const sortedResults=roomData?.response?.results[userId] || null;
        setSortedResult(sortedResults)
        const turnPlayer = roomData?.response?.players.find(
          (p) => p.is_turn === 1
        ...
        if (turnPlayer) {
          ...
          if (turnPlayer.user_id === Number(userId)) {
            ...
            eliminateTimerRef.current = setTimeout(() => {
              ...
              if (roomId && userId && !hasPickedCard) {
                console.log("⛔ Discard not done in time. Emitting eliminate.");
                DealsRummySocket.emit("eliminate", {
                  roomId,
                  userId: Number(userId),
                });
              }
        ...
        setEliminatedModalVisible(true);
        resetGame();
      } else if (task === "final_deal_result") {
        ...
        // );
        resetGame();
      } else if (task === "deal_Next_round_loading_timer") {
        const countdown = roomData?.response?.loading_timer?.countdown;
        ...
        setLockMessageShown(false);
        roomData?.response?.players?.forEach((player, i) => {
          sessionStorage.removeItem(`dlRummyPlayerTossCard${i + 1}`);
        });
      }
  ...
  const resetGame = () => {
    ...
    setTurnCountdown(0);
    sessionStorage.removeItem("roomData_deals");
  };
  ...
  const discardCardFromHand = (cardToDiscard) => {
    if (!DealsRummySocket?.connected) {
      toast.warn("⚠️ Server not connected yet, try again later");
      DealsRummySocket.connect();
      return;
    }
    ...
    DealsRummySocket.emit("deal_discardCard", payload);
  ...
  const discardCardFromHandFinally = (cardToDiscard) => {
    ...
    const hasAtLeast2Sequences =
       sortedResult?.pureSequences.length >= 1;
    const hasAtLeast1Pure = sortedResult.sets.length >= 1;

    if (!hasAtLeast2Sequences && !hasAtLeast1Pure) {
      toast.error(
        "You must have at least 2 sequences and 1 pure sequence before discarding."
      );
      return;
    }
  ...
  const pickFromDiscardPile = () => {
    ...
    DealsRummySocket.emit("deal_selectCardsFromDiscardedCards", payload);
    setDiscardPile(null);
  ...
  const pickFromLeftDeck = () => {
    ...
    DealsRummySocket.emit("deal_selectCardsFromLeftCards", payload);
    setHasPickedCard(true); // After successful pick
  };
  // declare
  const handleDeclare = () => {
    ...
    DealsRummySocket.emit("deal_declare", payload);
  ...
  return (
    <div className="w-full h-screen bg-[#1b0e26] flex flex-col items-center py-2 px-4 relative">
      ...
      {cardDiscardedFromHand && turnCountdown === 1 && (
        <div className="absolute top-2 text-sm text-bg2 font-bold">
          ...
          <div className="bg-white rounded-lg p-6 max-w-xl w-full">
            ...
            <button
              onClick={() => {
                ...
                navigate("/dlmenu"); // or reload game
              }}
