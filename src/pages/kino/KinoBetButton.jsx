...
function KinoBetButton({ handleClear, timeLeft, selectedRisk, selectedNumbers, setProfileRefresher }) {
    ...
    const gameBetHandler = async () => {
        ...
        const payload = {
            userid: userId,
            game_id: 17,
            risk_level: selectedRisk === "low" ? 1 : selectedRisk === "medium" ? 2 : selectedRisk === "medium" ? 3 : null,
                ...
                <div className='sm:h-36 w-full rounded-lg bg-[#005500] px-4 py-1 xsm:p-4'>
                    <div className='h-14 w-full rounded-3xl bg-[#569123] border-[0.5px] border-black pt-2'>
                        <div className='flex justify-between items-center'>
                            <div className='pl-4'>
                                ...
                                <div className='h-6 w-32 bg-[#2b7009] border-black border-[0.5px] text-center rounded-3xl text-white'>
                                    {betAmount}
                                </div>
                            </div>
                    ...
                    <div className='flex items-center justify-between'>
                        <button
                            ...
                            className={`relative ${(timeLeft > 0 && timeLeft < 11) ? "bg-gray" : "bg-gradient-to-tr from-[#448B02] to-[#5FAF09]"}  text-white shadow-lg drop-shadow-[0_4px_3px_rgba(0,0,0,0.3)] h-11 w-full font-serif font-bold text-[14px] m-2 rounded-2xl flex items-center justify-center`}>
                            BET
                        </button>
                    </div>
