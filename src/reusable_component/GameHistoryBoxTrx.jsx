      ...
      {gameHistoryData?.map((item, i) => {
        return (
          <div key={i} className='flex w-full border-b-[0.2px] border-border1 text-white text-[12px] bg-redLight p-2'>
            ...
            <NavLink to={`/lottery/trxwingo/tronscan2?blockid=${item?.blocknumber}`} className='w-[22%] flex flex-col justify-center items-center text-[12px]'>
              <p>
            ...
            <div className='flex h-[30px]  text-sm justify-center items-center w-[22%] relative'>
              ...
              {item?.result !== 0 && item?.result !== 5 && (
                <div className="flex items-center gap-1 text-[12px]">
                  <p className={`w-5 h-5 rounded-full text-white text-[12px] flex items-center justify-center ${[1, 3, 7, 9].includes(item?.result) ? "bg-green" : "bg-customred"}`} >{item?.result}</p>
                  <p className={`${[0, 1, 2, 3, 4].includes(item?.result) ? "text-bg3 text-[12px]" : "text-yellow"}`}>{[0, 1, 2, 3, 4].includes(item?.result) ? "S" : "B"}</p>
