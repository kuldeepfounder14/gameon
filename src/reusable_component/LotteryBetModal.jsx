...
function LotteryBetModal({ gameHistoryData, profileDetails, myHistory, bet_api, onClose, gameDetails }) {
    ...
    return (
        <>
            ...
            >
                <div onClick={(e) => e.stopPropagation()} className="w-full xsm:w-[400px] bg-red text-white rounded-t-3xl shadow-lg">
                    <div className={`${gameDetails?.colorCode === "rv" ? `bg-[#9B48DB]` : gameDetails?.colorCode === "gv" ? `bg-[#9B48DB]` : "bg-red text-blackLight"} h-[6.6rem] rounded-t-3xl`}>
                        ...
                        <div className="flex gap-1 mt-3 px-4">
                            {['1', '10', '100', '1000'].map((value, i) => (
                                <button
                                    ...
                                    className={`flex items-center justify-center text-xsm rounded-md h-7 px-2 ${balanceIndex === i ? '' : 'bg-redLight text-white'}`}
                        ...
                        {['X1', 'X5', 'X10', 'X20', 'X50', 'X100'].map((label, i) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`flex items-center justify-center text-xsm rounded-md h-7 px-2 ${quantityIndex === numericValue ? '' : 'bg-redLight text-white'
                                        }`}
                    ...
                    <div className="flex items-center mt-4 px-4">
                        <div onClick={() => setCheckAgreement(!checkAgreement)} className="flex items-center cursor-pointer bg-white rounded-full">
                            {checkAgreement ? (
                                <FaCheckCircle className='text-redLight' size={26} style={{ color: `[#B1835A]` }} />
                            ) : (
                                <FaRegCircle className='text-redLight' size={26} style={{ color: `[#B1835A]` }} />
                            )}
                        </div>
                        <label htmlFor="agree" className="text-white ml-2 text-xsm ">I agree</label>
                        <button onClick={togglePreSalesModal} className="ml-2 text-customred underline text-xsm ">{'<<Pre-sale rules>>'}</button>
                    </div>
